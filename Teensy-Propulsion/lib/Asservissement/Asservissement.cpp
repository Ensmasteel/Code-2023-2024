...
void MoveProfilesSetup::setup(){
    ...
    get(standard,true)->set(34000.0 , 0.01 , 0.01 , 0.01 , 0.003 , 0.03 , 0.6 , 0.6);           //translation
    get(standard,false)->set(2200.0 , 0.02 , 0.05 , 0.05 , 0.01 , 20*0.017 , 3.0/2.0*PI , 2*PI);   //rotation
...
void PID::setCurrentProfile(MoveProfileName profile){

    currentProfile = MoveProfilesSetup::get(profile, translation);
}
...
float PID::compute(float x, float dx, float xTarget, float dxTarget, float dt){
    ...
    float out = constrain(
        currentProfile->KP * diffX + currentProfile->KI * iTerm + currentProfile->KD * (dxTarget - dxF->out()), -1.0, 1.0);
