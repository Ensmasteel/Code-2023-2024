...
bool Lidar::AssemblePacket() {
    float last_angle = 0;
    // std::vector<PointData> tmp;
    int count = 0;
    Tofbf tofbfLd06(GetSpeed());
    mFrameTmp = tofbfLd06.NearFilter(mFrameTmp);
    std::sort(mFrameTmp.begin(), mFrameTmp.end(), [](PointData a, PointData b) { return a.angle < b.angle; });
    if (mFrameTmp.size() > 0) {
        mPopulateCallback(mFrameTmp);
        mFrameReady = true;
        mFrameTmp.clear();
        return true;
    }
...
/*!
        ...
        \retval     none
*/
Tofbf::Tofbf(int speed) { curr_speed = speed; }
